...
chrome.storage.sync.get(['selected_player_info'], (data) => {
    ...
    document.getElementById("highlight-attribute").innerHTML +=
        `            
         <button id="on-highlight" class="btn btn-default btn-margin2 float-right">
            <span class="glyphicon glyphicon-star" aria-hidden="true"></span> 포지션에 따른 중요 능력치 표시
        </button>
        `
    pos_arr.forEach((pos) => {
    ...
    abilities_arr.forEach((ab) => {
        Object.entries(abilities[ab]).forEach(entry => {
            ...
            document.getElementById(`${ab}-ability`).innerHTML += `<a id="attr-${key}" class="list-group-item">${dict[key]}: ${value}</a>`
        });
    });
    document.getElementById(`on-highlight`).addEventListener("click", () => {
        if (highlight_on) {
            ...
            document.getElementById(`on-highlight`).innerHTML = `<span class="glyphicon glyphicon-star" aria-hidden="true"></span> 포지션에 따른 중요 능력치 표시`
        } else {
            highlight_on = true
            important_abilities.forEach((attr) => {
            ...
            document.getElementById(`on-highlight`).innerHTML = `<span class="glyphicon glyphicon-star" aria-hidden="true"></span> 포지션에 따른 중요 능력치 끄기`
        }
    })
});